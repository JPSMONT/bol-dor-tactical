...
- **PWA install** — service worker (cache versioned — currently v16) caches the shell + pinned Leaflet/JSZip + Open-Meteo + MeteoSwiss responses, and map tiles (OpenSeaMap/CARTO) in a capped runtime cache, for offline race-day use; manifest with Little Johnka custom favicon and home-screen icons (192/512/apple-touch)
