...
- **Dashboard** — **mode-aware Race cockpit** at the top (PRD v6 Phase 0: glanceable tiles — nav, polar-efficiency, wind state, tactical bias, data-source tagged) with an **in-race maneuver-loss tracker** (Phase 1.5: race-scoped start/finish auto-arm + trim, GPS tack/gybe loss in boat-lengths, running polar %, persisted for Debrief), race countdown with 4-state transition (pre-race / race-morning / live elapsed clock / post-finish), race-day readiness checklist (tool/device only — boat & crew items removed), Race Sim banner, Quick VMG calculator, current-wind summary
