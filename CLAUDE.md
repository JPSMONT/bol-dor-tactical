...
- **Dashboard** — **mode-aware Race cockpit** at the top (PRD v6 Phase 0: glanceable tiles — nav, polar-efficiency, wind state, tactical bias + Maneuver/Risk placeholders, data-source tagged), race countdown with 4-state transition (pre-race / race-morning / live elapsed clock / post-finish), race-day readiness checklist (tool/device only — boat & crew items removed), Race Sim banner, Quick VMG calculator, current-wind summary
