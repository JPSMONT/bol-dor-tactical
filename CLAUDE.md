...
- **Rivals** — **venue-aware**. Bol d'Or: SuiviRegate KMZ for 4 archive years (auto-loads BOL25), 5 primary targets colour-highlighted (Leone/Pertuiset/Monachon/Rottet/Borter), tap-to-toggle visibility, sector arrival times table with `+X min` deltas, head-to-head distance-to-Bouveret SVG chart, bank-commitment chart, tack/gybe markers on map, time scrubber with Play/Pause, race-day banner (countdown/live/past), OSCAR link, per-rival "Analyse" deep dive (PPR, manoeuvre count, point-of-sail mix, outbound vs return VMC). Zugersee: **Kwindoo iframe embed** per Zugersee Cup 2026 event (Goldschäkel / Blauband / Rigi Anker / Chomer Bär), event header with countdown + manage2sail / host club links, paste-and-save Kwindoo URL per event (`zsc_events_v1`), **Yardstick class panel** with editable boat list and ±N window around Little Johnka YS 94 (`zsc_yardstick_v1`)
...
- **PWA install** — service worker (cache versioned — currently v18) caches the shell + pinned Leaflet/JSZip + Open-Meteo + MeteoSwiss responses, and map tiles (OpenSeaMap/CARTO) in a capped runtime cache, for offline race-day use; manifest with Little Johnka custom favicon and home-screen icons (192/512/apple-touch)
