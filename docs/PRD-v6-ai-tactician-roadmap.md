...
> **Build status (24 May 2026):** Phase 0 (Race cockpit) ✅, Phase 1.5 (in-race maneuver-loss tracker) ✅, Phase 2 v1 (in-app Debrief) ✅ — all shipped, behind the venue system. Phase 1 (live instruments) ⏸ **parked** pending hardware (`PRD-v6-live-instruments-spec.md` / `live-instruments-handoff-2026-05-24.md`). **Phase 1.6 (Tuning Log) ✅ shipped v1** (`PRD-v6-tuning-log-spec.md`). Phases 3–4 future.
...
**Exit criteria:** crew uses it at the Goldschäkel; entries persist; CSV joins to the GPS track by `t_ms`. **Spec:** `docs/PRD-v6-tuning-log-spec.md`. **Status:** v1 shipped 24 May — meets acceptance criteria; polar-diagram overlay + voice notes deferred to v2 (spec §8).
